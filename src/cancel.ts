...
import { DurableExecutorCancelledError } from './errors'
import { createConsoleLogger, type Logger } from './logger'

/**
 * A cancel signal is similar to an AbortController.
 * It allows you to check for cancellation and register a callback that will be called when the
 * signal is cancelled.
 *
 * @example
 * ```ts
 ...
 * `DurableExecutorCancelledError` if the signal is cancelled.
 *
 * @example
 * ```ts
 * const [cancelSignal, cancel] = createCancelSignal()
 ...
 * @param signal - An optional signal to cancel the promise.
 *
 * @returns A promise that will be rejected with a `DurableExecutorCancelledError` if the signal
 * is cancelled.
 *
 * @category Cancel
 */
export function createCancellablePromise<T>(
  ...
  return new Promise((resolve, reject) => {
    if (signal.isCancelled()) {
      reject(new DurableExecutorCancelledError())
      return
    }
